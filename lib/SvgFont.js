...
SvgFont.prototype = {
  ...
  _parseFont: function(content, options) {
    ...
    var
      ...
      fontInfo = fontDoc('font').first(),
      fontFontFace = fontDoc('font-face').first(),
      fontGlyphs = fontDoc('glyph'),

      fontHorizAdvX = fontInfo.attr('horiz-adv-x'),
      fontAscent = fontFontFace.attr('ascent'),
      fontUnitsPerEm = fontFontFace.attr('units-per-em') || 1000,

      scale = 1000 / fontUnitsPerEm;

    fontGlyphs.each(function(index, fontGlyph) {
      var
        d,
        unicode,
        name,
      ...
      unicode = fixedCharCodeAt(fontGlyph.attr('unicode'));
      code = unicode.toString(16);

      name = fontGlyph.attr('glyph-name') || code;
      width = fontGlyph.attr('horiz-adv-x') || fontHorizAdvX;
